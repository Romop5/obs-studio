...
struct color_inverse_filter_data {
 ...
 */
static const char *color_inverse_filter_name(void *unused)
{
	UNUSED_PARAMETER(unused);
	return obs_module_text("Color Inverse");
}
 ...
 */
static void color_inverse_filter_update(void *data, obs_data_t *settings)
{
	struct color_inverse_filter_data *filter = data;
 ...
 */
static void color_inverse_filter_destroy(void *data)
{
	struct color_inverse_filter_data *filter = data;
 ...
 */
static void *color_inverse_filter_create(obs_data_t *settings,
	obs_source_t *context)
{
	/*
	...
	* handle the source.
	*/
	struct color_inverse_filter_data *filter =
		bzalloc(sizeof(struct color_inverse_filter_data));

	/*
	 * By default the effect file is stored in the ./data directory that
	 * your filter resides in.
	 */
	char *effect_path = obs_module_file("color_inverse_filter.effect");
	 ...
	 */
	if (!filter->effect) {
		color_inverse_filter_destroy(filter);
		return NULL;
	}
	 ...
	 */
	color_inverse_filter_update(filter, settings);
	return filter;
}

/* This is where the actual rendering of the filter takes place. */
static void color_inverse_filter_render(void *data, gs_effect_t *effect)
{
	struct color_inverse_filter_data *filter = data;
 ...
 */
static obs_properties_t *color_inverse_filter_properties(void *data)
{
 ...
 */
static void color_inverse_filter_defaults(obs_data_t *settings)
{
 ...
 */
struct obs_source_info color_inverse_filter = {
	.id = "color_inverse_filter",
	.type = OBS_SOURCE_TYPE_FILTER,
	.output_flags = OBS_SOURCE_VIDEO,
	.get_name = color_inverse_filter_name,
	.create = color_inverse_filter_create,
	.destroy = color_inverse_filter_destroy,
	.video_render = color_inverse_filter_render,
	.update = color_inverse_filter_update,
	.get_properties = color_inverse_filter_properties,
	.get_defaults = color_inverse_filter_defaults
};
